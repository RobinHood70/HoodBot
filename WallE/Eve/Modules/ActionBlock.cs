	...
	internal sealed class ActionBlock : ActionModule<BlockInput, BlockResult>
	{
		...
		protected override void BuildRequestLocal(Request request, BlockInput input)
		{
			...
			request
				...
				.Add("anononly", (input.Flags & BlockFlags.AnonymousOnly) != 0)
				.Add("nocreate", (input.Flags & BlockFlags.NoCreate) != 0)
				.Add("autoblock", (input.Flags & BlockFlags.AutoBlock) != 0)
				.Add("noemail", (input.Flags & BlockFlags.NoEmail) != 0)
				.Add("hidename", (input.Flags & BlockFlags.Hidden) != 0)
				.Add("allowusertalk", (input.Flags & BlockFlags.AllowUserTalk) != 0)
