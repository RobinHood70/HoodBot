...
public abstract class ActionModule : IModule
{
	...
	protected Request CreateBaseRequest()
	{
		...
		return new Request(this.Wal.EntryPoint, this.RequestType, this.SiteVersion >= 128)
			...
			.AddIf("assert", wal.Assert!, wal.ValidStopCheckMethods.HasAnyFlag(StopCheckMethods.Assert) && !string.IsNullOrEmpty(wal.Assert))
			.AddIfNotNullIf("assertuser", wal.CurrentUserInfo?.Name, wal.ValidStopCheckMethods.HasAnyFlag(StopCheckMethods.UserNameCheck) && wal.SiteVersion >= 128)
