...
namespace RobinHood70.WallE.Base
{
	...
	public class SiteInfoExtensions
	{
		#region Constructors
		internal SiteInfoExtensions(string type, string? author, string? credits, string? description, string? descriptionMessage, IReadOnlyList<string>? descriptionMessageParameters, string? license, string? licenseName, string? name, string? nameMessage, string? url, string? version, string? versionControlSystem, DateTime? versionControlSystemDate, string? versionControlSystemUrl, string? versionControlSystemVersion)
		{
			this.Type = type;
			this.Author = author;
