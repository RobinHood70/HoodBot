...
[method: JobInfo("Convert Fixed Namespace Numbers")]
internal sealed class ConvertFixedNamespaceNumbers(JobManager jobManager) : ParsedPageJob(jobManager)
{
	...
	{
		var title = TitleFactory.FromUnvalidated(this.Site, "Oblivion Mod:Oscuro's Oblivion Overhaul/Items");

		var nsList = new UespNamespaceList(this.Site);
		if (nsList.FromTitle(title) is not UespNamespace uespNs)
		{
			throw new InvalidOperationException("Title is invalid.");
		}

		var basePageName = uespNs.GetTitle(string.Empty).PageName;
		this.Pages.GetPageLinks([title]);
		for (var i = this.Pages.Count - 1; i >= 0; i--)
		{
			if (!this.Pages[i].Title.PageName.StartsWith(basePageName, StringComparison.CurrentCulture))
			{
