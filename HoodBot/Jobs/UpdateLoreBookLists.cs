...
public class UpdateLoreBookLists(JobManager jobManager) : EditJob(jobManager)
{
	...
	protected override void PageLoaded(Page page)
	{
		...
		var first = parser.IndexOf<TemplateNode>(node => node.GetTitle(this.Site) == templateTitle);
		var last = parser.LastIndexOf<TemplateNode>(node => node.GetTitle(this.Site) == templateTitle);
		if (first == -1)
		{
