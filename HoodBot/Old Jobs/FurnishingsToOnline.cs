namespace RobinHood70.HoodBot.Jobs
{
	...
	public class FurnishingsToOnline : EditJob
	{
		...
		private static string RemoveTemplate(WikiNodeCollection nodes, int templateIndex)
		{
			...
			var placeHolder = nodes.FindIndex<TemplateNode>(node => node.TitleValue.PageNameEquals("Placeholder"));
			if (placeHolder != -1)
			{
		...
		private void FilesPageLoaded(object sender, Page page)
		{
			...
			var templateIndex = originalParser.FindIndex<TemplateNode>(template => template.TitleValue.PageNameEquals("Furnishing Summary"));
			if (templateIndex > -1)
			{
				var originalTemplate = (TemplateNode)originalParser[templateIndex];
				...
				var newTemplate = (TemplateNode)parser.Factory.TemplateNodeFromParts("Online Furnishing Summary\n");
